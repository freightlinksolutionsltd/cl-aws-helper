...
function publishEvent(
	...
	if (size > 256) {
		console.log(
			`=> AWS Event bridge: The payload for passport ${data.passportNo} is ${size}KB which exceeds the 256KB payload limit`
		);
	}
...
async function moveObject(fromBucket, toBucket, fileKey, secretKey, accessId) {
	...
	console.log(
		`Attempting s3 copy of "${fileKey}" from "${fromBucket}" to "${toBucket}"`
	);

	const copyRes = await s3(secretKey, accessId)
	...
	const deleteRes = await s3(secretKey, accessId)
